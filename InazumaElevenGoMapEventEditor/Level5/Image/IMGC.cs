...
namespace InazumaElevenGoMapEventEditor.Level5.Image
{
    public static class IMGC
    {
        public static Bitmap ToBitmap(Stream inputStream)
        {
        ...
        {
            using (var table = new BinaryDataReader(new MemoryStream(tile)))
            using (var tex = new BinaryDataReader(new MemoryStream(imageData)))
            {
