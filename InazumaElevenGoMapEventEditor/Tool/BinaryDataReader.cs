...
namespace InazumaElevenGoMapEventEditor.Tool
{
    public class BinaryDataReader : IDisposable
    {
        ...
        public string[] ReadMultipleSting(int count, Encoding encoding)
        {
            return Enumerable.Range(0, count).Select(x => ReadString(encoding)).ToArray();
        }
    }
}
